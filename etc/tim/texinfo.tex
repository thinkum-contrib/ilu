...
\def\begindoublecolumns{\begingroup
  ...
  %
  % Double the \vsize, plus a little extra so we'll be sure to have
  % enough text to fill up the page (but not too much).  (We don't need
  % a separate register here, since nobody clobbers \vsize.)
  \vsize = 2\vsize
...
\catcode `\#=\other
\catcode `\&=\other
\catcode 43=\other % 43 is the plus character ``+''

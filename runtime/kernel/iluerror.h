...
/*
*/
/* $Id: iluerror.h,v 1.32 1999/08/09 23:35:15 janssen Exp $ */
...
The error type of an error (a small integer value) may be accessed
with

	ilu_cardinal ILU_ERR_TYPE(ilu_Error err);

The description associated with a minor code can be gotten by calling

	extern const char * ilu_GetMinorDescrFromCodes(	ilu_cardinal et,
							ilu_cardinal minor);

which returns NIL if no description is available; the "et" parameter
is the error type of the error.
...
Surrogate-side runtimes need a way of mapping protocol exceptions
received from the wire to ilu_Error values.  The function

  void ilu_MapProtocolExceptionToError (ilu_ProtocolException pe,
					ilu_Error *err)

provides such a mapping in a standard fashion.  The "err" argument should
be the address of a value of type ilu_Error; it serves as the output value
from the function.
...
#define ILU_DEF_ERR(id,xdescrip) ILU_DEF_ERRG(id,xdescrip,0,0)

#define ILU_DEF_ERRM(id,xdescrip,mid) \
static const char *ilu_##id##_minors[] = { ILU_##mid##_MINORS }; \
ILU_DEF_ERRG(id,xdescrip,ilu_##id##_minors,ilu_##id##_nextMinor - ILU_VMCID_BASE)

#define ILU_DEF_ERRG(id,xdescrip,minors,nMinors) \
static void ilu_EF_##id (ilu_Error*); \
ilu_ErrorTypeDetailsStruct ilu_ETS_##id = { # id , xdescrip, ilu_EF_##id, nMinors, minors }; \
...
ILU_PUBLIC const char * ilu_GetMinorDescrFromCodes(unsigned et,
						   unsigned long minor);
...
#define ILU_ERR_TYPE(err)	 ((ilu_cardinal)((err).ilu_type))
